...
export default {
  ...
  methods: {
    ...
    initBeanlist() {
      this.loading = true;
      let url = '/' + this.flag + '/sellrule/?page=' + this.page + '&size=' + this.size;
    ...
    getDraft() {
      this.getRequest('/' + this.flag + '/sellrule/getDraft').then(resp =>{
        if (resp) {
          if (resp.obj) { // 存在草稿
    ...
    doAddBean() {
      if (this.bean.id) {
        this.$refs['beanForm'].validate(valid => {
          if (valid) {
            this.putRequest('/' + this.flag + '/sellrule/', this.bean).then(resp => {
              if (resp) {
        ...
        this.$refs['beanForm'].validate(valid => {
          if (valid) {
            this.postRequest('/' + this.flag + '/sellrule/', this.bean).then(resp => {
              if (resp) {
                this.dialogVisible = false;
                this.initBeanlist();
              }
    ...
    toRunBean(id) {
      this.$confirm('确认将该卖出规则投入运行中？', '提示', {
        ...
        this.getRequest('/' + this.flag + '/sellrule/run/' + id).then(resp => {
          if (resp) {
            this.getDraft();
            this.initBeanlist();
          }
