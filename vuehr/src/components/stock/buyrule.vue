...
export default {
  ...
  methods: {
    ...
    initBeanlist() {
      this.loading = true;
      let url = '/' + this.flag + '/buyrule/?page=' + this.page + '&size=' + this.size;
    ...
    getDraft() {
      this.getRequest('/' + this.flag + '/buyrule/getDraft').then(resp =>{
        if (resp) {
          if (resp.obj) { // 存在草稿
    ...
    doAddBean() {
      if (this.bean.id) {
        this.$refs['beanForm'].validate(valid => {
          if (valid) {
            this.putRequest('/' + this.flag + '/buyrule/', this.bean).then(resp => {
              if (resp) {
        ...
        this.$refs['beanForm'].validate(valid => {
          if (valid) {
            this.postRequest('/' + this.flag + '/buyrule/', this.bean).then(resp => {
              if (resp) {
                this.dialogVisible = false;
                this.initBeanlist();
              }
    ...
    toRunBean(id) {
      this.$confirm('确认将该买入规则投入运行中？', '提示', {
        ...
        this.getRequest('/' + this.flag + '/buyrule/run/' + id).then(resp => {
          if (resp) {
            this.getDraft();
            this.initBeanlist();
          }
