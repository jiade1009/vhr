...
export default {
  ...
  methods: {
    ...
    initBeanlist() {
      this.loading = true;
      let url = '/' + this.flag + '/weeklylineresult/?page=' + this.page + '&size=' + this.size;
    ...
    reloadWeekly(e) {
      ...
      if (this.flag=='stock' || this.flag=='qtstock') {
        ...
        msg = "如果时间未超过凌晨5点，则生成上两个交易日的周线；如果时间超过凌晨5点，则生成上一个交易日的周线；" +
            "此操作将需要时间生成U股周线历史数据和EMA数据，是否继续?(针对北京时间而言)"
      }
      ...
      this.$confirm(msg, '提示', {
        ...
        this.disabled = true;
        this.postRequest('/' + this.flag + '/weeklylineresult/', data).then(resp => {
          this.disabled = false
          if (resp) {
            this.initBeanlist();
          }
    ...
    getEmaList(row) {
      ...
      let url = '/' + this.flag + '/weeklylineemaresult/byweekly?wid='+row.id
      this.getRequest(url).then(resp => {
        this.loading = false;
        if (resp) {
          row.emalist = resp;
    ...
    generateEma(row) {
      this.$confirm('确认用该历史行情，生成新的EMA数据吗？', '提示', {
        ...
        this.putRequest('/' + this.flag + '/weeklylineemaresult/newema?wid=' + row.id).then(resp => {
          if (resp) {
            this.initBeanlist();
          }
        })
    ...
    runBuyRule(row) {
      this.$confirm('确认采用该EMA数据运行购买策略吗？', '提示', {
        ...
        this.putRequest('/' + this.flag + '/weeklylineemaresult/runBuyRule?emaid=' + row.id).then(resp => {
          if (resp) {
            // this.initBeanlist();
            //更新ema list信息
